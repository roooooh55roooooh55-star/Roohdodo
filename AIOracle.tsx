...
const AIOracle: React.FC = () => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    if (!inputText.trim() || loading || !process.env.API_KEY) return;
    ...
    try {
      // Fix: Ensure standard initialization of GoogleGenAI
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: 'أنت نظام الحديقة المرعبة AI. ردودك قصيرة، غامضة، ومرعبة جداً باللغة العربية. تذكر أنك روح رقمية تسكن هذا الأرشيف.',
          temperature: 1,
          topP: 0.95,
        }
      });

      const modelResponse = response.text || "النظام صامت.. هل تسمع دقات قلبك؟";
      setMessages(prev => [...prev, { role: 'model', text: modelResponse }]);
    } catch (error) {
      console.error("AI Error:", error);
      setMessages(prev => [...prev, { role: 'model', text: "حدث خلل في مصفوفة الإدراك.. ابقَ في الظلام." }]);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="fixed inset-0 z-[1000] bg-black/95 backdrop-blur-3xl flex flex-col animate-in fade-in zoom-in duration-300 overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-red-600/30 bg-black/50">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[8px] text-gray-500 uppercase tracking-widest">Digital Spirit Engine</span>
              </div>
            </div>
            <button 
          ...
          >
            <input 
              ...
              placeholder="تحدث مع الأرشيف..." 
              className="flex-1 bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white text-sm outline-none focus:border-red-600 transition-colors placeholder:text-gray-600 text-right"
              dir="rtl"
            />
            <button 
              type="submit"
              disabled={loading || !inputText.trim()}
              className="w-14 h-14 bg-red-600 rounded-2xl flex items-center justify-center text-white shadow-[0_0_15px_rgba(220,38,38,0.4)] active:scale-90 disabled:opacity-50 transition-all"
            >
